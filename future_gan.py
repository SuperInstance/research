"""Future-GAN: army of APIs + JEV judge for canon discovery.

Hypothesis: multiple LLMs each generate lore variants for a substrate walker seed.
JEV (the judge) scores canon-worthiness. Pick the best.

Architecture:
- 3 generators: ZAI glm-5.3-flash, DeepSeek deepseek-chat, DeepInfra Llama-3.1-8B
- 1 judge: JEV (Typesafe.ai System One)
- 1 discriminator: distinct voice check
- 1 selection: top-k by composite JEV score
"""
import sys
import json
import time
...
from api_call import call_zai, call_deepseek, call_deepinfra, call_jev
...
VOICES = {
    ...
    "oracle": (
        ...
        "End on the witness becoming a listener who can hear the oracle's next prophecy."
    ),
}

SEEDS = [70051917, 4685000, 3289967, 57322595, 90625407]
...
def gen_lore(voice: str, seed: int) -> str:
    """Generate one lore via DeepInfra (most reliable)."""
    prompt = VOICES[voice].format(seed=seed, theme=THEMES.get(seed, "the substrate remembered itself"))
    try:
...
def score_lore(lore: str) -> dict:
    ...
    try:
        ...
        return {"canon_worthy": 0, "distinct_voice": 0, "doctrine_anchor": 0, "composite": 0, "error": str(e)}


def run():
    """Run future-GAN across all seeds × voices."""
    print("=== Future-GAN Canon Discovery ===\n")
    results = []

    for seed in SEEDS:
        for voice in VOICES:
            print(f"Seed {seed}, voice {voice}:")
            t0 = time.time()
            lore = gen_lore(voice, seed)
            if not lore or len(lore) < 100:
                print(f"  SKIP: lore too short ({len(lore) if lore else 0} chars)")
                continue
            ...
            print(f"  lore={len(lore)}c, canon={scores['canon_worthy']:.2f}, "
                  f"distinct={scores['distinct_voice']:.2f}, doctrine={scores['doctrine_anchor']:.2f}, "
                  f"composite={scores['composite']:.2f}")
            print(f"  gen={t1-t0:.1f}s, score={t2-t1:.1f}s")

    # Sort by composite
    results.sort(key=lambda r: -r['scores']['composite'])

    # Save
    out_path = "/workspace/research/FUTURE_GAN_RESULTS.json"
    ...
    print(f"\nCanon pass rate (≥0.7): {canon_passes}/{len(results)}")
    print(f"Distinct pass rate (≥0.7): {distinct_passes}/{len(results)}")
    print(f"Composite ≥0.6: {composite_passes}/{len(results)}")
